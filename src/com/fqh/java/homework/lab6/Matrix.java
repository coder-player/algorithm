package com.fqh.java.homework.lab6;
...
public class Matrix {
	...
	public Matrix matrixMul(Matrix anotherMatrix){
		if(width() != anotherMatrix.height()){
			throw new ArrayIndexOutOfBoundsException("参数不合法 当前矩阵的列不等于传入矩阵的行");
		}
		int finalVal = 0;
		Matrix result = new Matrix(height(),anotherMatrix.width());
		//Add your code here
		for (int i = 0; i < result.h; i++) {
			for (int j = 0; j < this.h; j++) {
				for (int k = 0; k < anotherMatrix.h; k++) {
					//计算当前矩阵第i行乘传入的矩阵第j列的值的和
					int val = (Integer) this.get(i, k) * (Integer) anotherMatrix.get(k, j);
					finalVal += val;
				}
				result.set(i, j,finalVal);
				finalVal = 0;
			}
		}
		return result;
